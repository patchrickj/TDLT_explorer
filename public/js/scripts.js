...
require([
  ...
  var climbingSymbol = new PictureMarkerSymbol('\\img\\symbols\\climbing.png', symbolSize[0], symbolSize[1]);
	var buildingSymbol = new PictureMarkerSymbol('\\img\\symbols\\building.png', symbolSize[0], symbolSize[1]);
	var parkingSymbol = new PictureMarkerSymbol('\\img\\symbols\\parking.png', symbolSize[0], symbolSize[1]);
	var peakSymbol = new PictureMarkerSymbol('\\img\\symbols\\peak.png', symbolSize[0], symbolSize[1]);
	var picnicSymbol = new PictureMarkerSymbol('\\img\\symbols\\picnic.png', symbolSize[0], symbolSize[1]);
	var poiSymbol = new PictureMarkerSymbol('\\img\\symbols\\poi.png', symbolSize[0], symbolSize[1]);
	var trailheadSymbol = new PictureMarkerSymbol('\\img\\symbols\\trailhead.png', symbolSize[0], symbolSize[1]);
	var viewpointSymbol = new PictureMarkerSymbol('\\img\\symbols\\viewpoint.png', symbolSize[0], symbolSize[1]);
  ...
  var tdltLands = new FeatureLayer(tdlt_lands_url, {
 	...
 	var tdltTrails = new FeatureLayer(tdlt_trails_url, {
 		id: "tdltTrails",
 		mode: FeatureLayer.MODE_SNAPSHOT,
 		outFields: ["NAME", "Status", "MT_BIKE", "HORSE", "Mileage"],
  ...
  //Create Legends
	...
	query.outFields = ["NAME", "TYPE", "Acres"];
  ...
  map.on("click", function(event) {
  	...
  	//make sure layer is visible and query set to lands
		if (radioValue === "lands" && landVis) {
			...
			query.geometry = pointToExtent(map, event.mapPoint, .5);
			tdltTrails.selectFeatures(query, FeatureLayer.SELECTION_NEW, function (selection) {
				if (selection.length === 1) {
					map.infoWindow.setFeatures(selection);
		 			map.infoWindow.show(selection)
				}
